...
describe('createBoundSelector', () => {
  ...
  test('should implement binding to partial props', () => {
    const marryAndMessageSelector = createBoundSelector(personAndMessageSelector, {
      personId: 1,
    });
    ...
    const personAndHelloSelector = createBoundSelector(personAndMessageSelector, {
      messageId: 100,
    });
  ...
  test('should implement binding to full props', () => {
    const marryAndHelloSelector = createBoundSelector(personAndMessageSelector, {
      personId: 1,
      messageId: 100,
    });
  ...
  test('should not support binding of optional values to non-optional props', () => {
    ...
    const selector = (state: State, props: Props) => JSON.stringify(state) + JSON.stringify(props);
  ...
  test('should generate selector name', () => {
    ...
    expect(marrySelector.selectorName).toMatchInlineSnapshot(`"personSelector (personId -> [*])"`);
    ...
    (
      personAndMessageNamedSelector as NamedParametricSelector<unknown, unknown, unknown>
    ).selectorName = 'personAndMessageNamedSelector';

    const marryAndHelloSelector = createBoundSelector(personAndMessageNamedSelector, {
      personId: 1,
      messageId: 100,
    });
  ...
  describe('integration with re-reselect', () => {
    ...
    test('should decorate getMatchingSelector and removeMatchingSelector of dependency', () => {
      ...
      let selectorInstance = marrySelector.getMatchingSelector?.(commonState, expect.anything());
      expect(selectorInstance).toBeUndefined();

      marrySelector(commonState);
      selectorInstance = marrySelector.getMatchingSelector?.(commonState, expect.anything());
      ...
      const key = marrySelector.keySelector(commonState, expect.anything()) as unknown;
      expect(key).toBe(1);
    });
  ...
  test('should return default key selector if all selectors are bounded', () => {
    ...
    const selector = createCachedSelector([], () => {})({
      keySelector: stringComposeKeySelectors(firstPropSelector, secondPropSelector),
    });
