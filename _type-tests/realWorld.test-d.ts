...
const getPersonName = createPathSelector(
  (state: State, props: { personId: number }) =>
...
const getNameByUserId = createAdaptedSelector(
  getPersonName,
  (props: { userId: number }) => ({ personId: props.userId }),
);
...
const getPersonMessages = createChainSelector(
  createPropSelector<{ personId: number }>().personId(),
)
  .chain((personId) => (state: State) =>
    Object.values(state.messages.data).filter(
...
expectTypeOf(
  getPersonMessages(stateFixture, { personId: 1 }),
...
const getPersonViewModel = createCachedStructuredSelector({
  person: (state: State, props: { personId: number }) =>
    state.persons.data[props.personId],
  name: getPersonName,
...
expectTypeOf(
  getPersonViewModel(stateFixture, { personId: 1 }),
).toEqualTypeOf<{
  person: Person;
  name: string;
  messages: Message[];
